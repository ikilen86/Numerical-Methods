...
class myTimer{
	...
	public:
		...
		//! Zero out counter and recorded time information, but leave names/identifiers intact
		void reset()
		{
			counter = 0;
			real_time = 0;
		...
		void stop()
		{
			...
			//	double  diff = ((double)t2.tv_sec + (double)t2.tv_usec * .000001 - (double)t1.tv_sec - (double)t1.tv_usec * .000001);
				...
				cout << " " << endl;
				cout << "ERROR" << endl;
				cout << "Asking for stop() before calling start()" << endl;
				exit(-1);
			}
		}
		
		//! Return current elapsed time
		/*! No other changes to the timer class*/
		double curr_time()
		{
			if (start_timer)
			{
				clock_gettime(CLOCK_REALTIME,&t2);
				return ((double)t2.tv_sec + (double)t2.tv_nsec * 1e-9 - (double)t1.tv_sec - (double)t1.tv_nsec * 1e-9);
				...
				cout << " " << endl;
				cout << "ERROR" << endl;
				cout << "Asking for lap() before calling start()" << endl;
				exit(-1);
			}
		}
		...
		void Print(int step)
		{
			if (start_timer)
			{
				...
				cout << " " << endl;
				cout << "ERROR" << endl;
				cout << "Asking for Print() before calling stop()" << endl;
				exit(-1);
			}
		...
		void Print_table(int step)
		{
			if (start_timer)
			{
				...
				cout << " " << endl;
				cout << "ERROR" << endl;
				cout << "Asking for Print_table() before calling stop()" << endl;
				exit(-1);
			}
		...
		void Print_minimal(int step, double parent_real_time, double parent_counter)
		{
			if (start_timer)
			{
				...
				cout << " " << endl;
				cout << "ERROR" << endl;
				cout << "Asking for Print_minimal() before calling stop()" << endl;
				exit(-1);
			}
...
class myTimerCentral{
	...
	private:
		...
		void reset_timer(struct timer_tree *tmp)
		{
			if (tmp != NULL)
			{
				tmp->p_timer->reset();
				reset_timer(tmp->next_same_level);


				if (tmp->next_sub_level != NULL)
				{
					for(int i = 0; i < MAX_SUB_TIMERS; i++)
					{
						reset_timer(tmp->next_sub_level[i]);
					}
				}
			}
		}
